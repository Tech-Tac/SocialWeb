...
<div class="post card my-4 shadow" id="post_{{ $post->id }}">
	<div class="card-header">
		<div class="row px-1 gap-2">
			...
			<div class="col p-0">
				@if ($post->group)
				<a class="fw-bold text-body-emphasis" href="{{ route('groups.show', $post->group) }}">{{ $post->group->name
					}}</a>
				&gt;
				@endif
				<a class="fw-bold text-body-emphasis" href="{{ route('users.show', $post->user) }}">{{ $post->user->name }}</a>

				<time class="text-secondary float-end" datetime="{{ $post->created_at }}" title="{{ $post->created_at }}">{{
					$post->created_at->diffForHumans() }}</time>
				<br>
				<h5 class="card-title d-inline">{{ $post->title }}</h5>
				@if ($post->created_at != $post->updated_at)
				<span class="text-secondary fs-6">
					edited <time datetime="{{ $post->updated_at }}" title="{{ $post->updated_at }}">{{
						$post->updated_at->diffForHumans() }}</time>
				</span>
				@endif
			</div>
